...
describe('Trie', () => {
  ...
  describe('INSERT', () => {
    ...
    it('should add words to the trie', () => {
      ...
      expect(
        completion.root.children['p'].children['i'].children['z']
      ).to.exist;
      expect(
        completion.root.children['p'].children['i'].children['a']
      ).to.exist;
      expect(
        completion.root.children['d'].children['o'].children['g'].endOfWord
      ).to.be.true;
    });
  ...
  describe('findLastNode', () => {
    it('should return the node corresponding to the last letter of a string', 
      () => {
        ...
        expect(
          completion.findLastNode('pi')
        ).to.exist.and.be.an.instanceOf(Node);

        const childrenKeys = Object.keys(
          completion.findLastNode('pi').children
        );

        expect(childrenKeys).to.eql(['z']);
        expect(
          completion.findLastNode('pi').children.z
        ).to.exist.and.be.an.instanceOf(Node);
      }
    );
  });

...
export default class Trie {
  ...
  addWordToTrie(node, word) {
    ...
    // if the current node (which is initially the root) doesn't have a key in its children object corresponding to the current letter, create it with a value equal to a new node
    if (!node.children[letter]) {
      node.children[letter] = new Node();
    }

    //if the current letter is the last in the word being added and the word has not been previously added to the node, increase the tries count and set the endOfWord property of the node to true;
    ...
    // if there are more letters to account for beyond the current letter, feed it back in to this method with the new node as the node corresponding to the current letter and remainder of the word (not including the current letter)
    if (word.length > 1) {
  ...
  insert(word) {
    // call the addWordToTrie method and pass in 'this' (i.e. the root), and the word
    this.addWordToTrie(this, word.toLowerCase());
  }
